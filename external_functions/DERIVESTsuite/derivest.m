...
%
%        DEFAULT: 100
%                 MRW: changed (see comment in code)
%
...
%par.NominalStep = x0;  % MRW: max with 0.02 allows positive values to become negative (for positive rate values)
par.NominalStep = max(x0,1);  % MRW: for log rate values
...
if isempty(par.FixedStep)
  ...
  delta = par.MaxStep*par.StepRatio .^(0:-1:-25)';  %MRW: trying to go to -35 makes the algorithm unstable, so back to normal value of 25
  ndel = length(delta);
else
