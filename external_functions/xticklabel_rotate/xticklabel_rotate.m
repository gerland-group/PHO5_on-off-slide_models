function hText = xticklabel_rotate(XTick,rot,my_offset,varargin)
...
if strcmp(xAxisLocation,'bottom')  
    set(hText,'Rotation',rot,'HorizontalAlignment','left',varargin{:})  % Michael Wolff: changed from right to left
else  
...
set(hText,'units','normalized');
for loop= 1:length(hText),
    set(hText(loop),'position',get(hText(loop),'position')+[my_offset 0])  ;  % Michael Wolff: shift with my_offset
end
